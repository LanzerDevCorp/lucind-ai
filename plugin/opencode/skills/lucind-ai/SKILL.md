...
- Advance to wave N+1 only after wave N's `lucind-ai run` exits 0 (every lane `done`, none in `reverted_ids`). Halt remaining waves on non-zero exit; do not skip failed nodes. `lucind-ai split` prints the commands; it does not schedule them.
...
- Never infer unbuilt capability from the domain vocabulary. Today, feature attempts, Ownership Leases, compare-and-swap Promotion, overlap gating, recovery, reconciliation, feature retire/re-anchor (`feature disable`), no-redispatch integration retry (`integrate retry`), and ultrafixer-based Defect Assessment and Defect Record persistence (`lucind-ai defect record`/`list`/`decline`, dispatched via the ultrafixer packet template — see `references/coordination/dependencies-defects.md`) are production-wired; a separate prepared-fix-Change-with-Dependencies Remediation Proposal, External Work Item creation, and Claude Agent Teams launching are not.
