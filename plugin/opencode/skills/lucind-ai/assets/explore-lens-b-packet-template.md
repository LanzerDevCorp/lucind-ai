---
id: explore-<change-id>-lens-b
executor: agy
routed_by: capabilities and scenarios lens of the three-lens explore fan-out
model: <model, e.g. gemini-3.7-flash-high>
allowed_paths: ["openspec/changes/<change-id>/explore-lens-b.md"]
---

# Packet explore-<change-id>-lens-b

**Tier:** B (auto-merge after audit)
**Worktree:** ../<repo>-worktrees/explore-<change-id>-lens-b  ·  **Branch:** lucind/explore-<change-id>-lens-b

## Goal

Produce `openspec/changes/<change-id>/explore-lens-b.md`: the capability impact, user journeys, concrete scenarios, and measurable success criteria for this change.
...
The exploration for `<change-id>` is initiating. Lens A and lens C run in parallel against the same codebase and write to different files, so no lane races another.

## Preconditions

- `openspec/changes/<change-id>/` exists (or the packet `## Context` states the change goal).
- `openspec/changes/<change-id>/explore-lens-b.md` does not yet exist.
...
Write exactly this skeleton to `openspec/changes/<change-id>/explore-lens-b.md`:

```markdown
# Explore Lens B — Capabilities & Scenarios: <Change Title>
...
`openspec/changes/<change-id>/explore-lens-b.md` only. Create no other file.
...
```
./lucind-lane-check.sh --file openspec/changes/<change-id>/explore-lens-b.md --budget 1000 \
...
```
./lucind-lane-check.sh --file openspec/changes/<change-id>/explore-lens-b.md
```
...
<Ground-truth facts with file:line references: the existing capabilities,
user flows, interfaces, and any decision the human has already made in
