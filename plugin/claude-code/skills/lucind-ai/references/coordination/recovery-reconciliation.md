...
4. Remove the blocked Lane's preserved worktree and lane branch before reusing the same ID. `lucind-ai worktree cleanup --lane <id>` only removes a clean worktree (or use `--force` if uncommitted changes are non-recoverable); it does not delete the `lucind/<id>` branch, so deleting the branch is a separate manual `git branch -D lucind/<id>` step.
