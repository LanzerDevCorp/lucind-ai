...
This is the packaged, execution-facing projection of root `CONTEXT.md` for consumers that do not have the source checkout. `internal/packet/packet_test.go` requires the canonical glossary below to match the root glossary byte for byte. Change terms in `CONTEXT.md` first, then update this projection in the same Change.
