...
Reusable templates omit live `feature`, `parent_ref`, `base_sha`, and `expected_parent_sha`. At wave dispatch the Orchestrator late-binds all four fields onto the packet copies it passes to `lucind-ai run`. Every packet in one dispatched batch must then name identical values for those four keys. The Orchestrator creates the git parent branch and separately records it with `lucind-ai feature create`; the command writes the ledger row and does not create the branch. Lanes do not create or move parent refs.
