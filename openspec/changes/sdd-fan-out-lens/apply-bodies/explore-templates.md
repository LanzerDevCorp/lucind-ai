# Packet sdd-fan-out-lens-explore-templates

## Goal

Create the four explore fan-out packet templates so three disjoint lens packets and one integrated-tree synthesizer satisfy the accepted planning protocol and the integrated RED contract tests.
...
- Packet `sdd-fan-out-lens-contract-tests` has integrated tests that parse the explore templates and check phase-local lens-path disjointness.
...
- `plugin/claude-code/skills/lucind-ai/assets/explore-lens-a-packet-template.md`
- `plugin/claude-code/skills/lucind-ai/assets/explore-lens-b-packet-template.md`
- `plugin/claude-code/skills/lucind-ai/assets/explore-lens-c-packet-template.md`
- `plugin/claude-code/skills/lucind-ai/assets/explore-synthesis-packet-template.md`
...
- Do not edit tests, `SKILL.md`, propose/design templates, application code, runtime/DAG/CLI code, or build scripts.
...
- Owned tasks, quoted from `tasks.md`: **1.1, 1.2** (`openspec/changes/sdd-fan-out-lens/tasks.md:25-26,56`). Strict TDD is active: make the integrated RED assertions pass for this owned slice without changing tests.
- The design requires dedicated `explore-lens-{a,b,c}-packet-template.md` and `explore-synthesis-packet-template.md` files at `openspec/changes/sdd-fan-out-lens/design.md:11-16`.
- Lens ownership is fixed at `openspec/changes/sdd-fan-out-lens/design.md:83`: A owns problem and candidates; B owns capabilities and scenarios; C owns risks, trade-offs, and spikes.
...
- The synthesizer must produce canonical `explore.md` plus sectioned synthesis notes, verify every `file:line`, and keep its canonical ceiling strictly below the sum of the three lens ceilings (`openspec/changes/sdd-fan-out-lens/design.md:32-44,59-65`).
- The work-unit table deliberately grants individual files, not the parent `assets/` directory (`openspec/changes/sdd-fan-out-lens/tasks.md:51,56`). This conservative literal-file boundary prevents same-wave overlap with propose templates.
...
- [ ] The integrated explore-focused test/subtest passes and proves all four files parse with `packet.Parse`, each admits through `legacy_main: true` or complete feature fields, and the three lens `allowed_paths` are pairwise disjoint. Attach command and output.
- [ ] Each lens body names its exclusive slice, required reading, output skeleton, sibling ownership, and word ceiling; the synthesis body names all three drafts, canonical `explore.md`, synthesis notes, citation verification, and a canonical ceiling strictly below the lens-ceiling sum. Evidence cites the created file lines.
...
- A required precondition is false or the RED dependency has not integrated the expected explore contract tests.
...
- Focused verification fails outside this packet's scope; failures solely owned by concurrent propose/docs packets must be reported as such, not repaired here.
