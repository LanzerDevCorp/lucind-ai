...
- [x] 3.2 Create `internal/phasespec/phasespec.go`: parse `sdd-status --json`, gate synthesis until required lens IDs are accepted and merged, write `openspec/changes/<change>/<phase>.md`. No `cmd` import; admission stays in `admitDispatchBatch` (`design.md:28-36,106`; `explore.md:282-285`). Was reopened by verify.md (first pass) findings 3, 4, 8 — lens state, canonical filenames, disk-existence check; fixed by remediation 5.2/5.3/5.7 and round-2 remediation for the filename convention; confirmed fixed by round-2/3 dual re-verify.
- [x] 3.3 Modify `cli.go` string switch (`:142-168`) to register `lucind-ai phase <name>` and dispatch through `phasespec` (`design.md:28-36,107`). Accept still never demotes (`cli.go:684-687`). Was reopened by verify.md (first pass) finding 2 — `phaseDispatch` never dispatched a real synthesis lane; fixed by remediation 5.2, confirmed fixed by round-2/3 dual re-verify (`TestPhaseSubcommandDispatchesSynthesisWhenLensesMerged`).
