---
id: verify-skill-anchoring-guardrails-cursor-agent
executor: cursor-agent
routed_by: qualitative verification of spec compliance, edge cases, and test quality — cursor-agent judge (second of the dual-dispatch pair; this is the single approved exception to the AGY-only Execution Strategy, kept specifically for adversarial cross-executor verification per human decision)
read_only: true
...
# Packet verify-skill-anchoring-guardrails-cursor-agent

**Tier:** A (human merge)
**Worktree:** ../lucind-ai-worktrees/verify-skill-anchoring-guardrails-cursor-agent · **Branch:** lucind/verify-skill-anchoring-guardrails-cursor-agent
...
You are the adversarial cross-check on the `agy` judge running the same evaluation concurrently in a separate worktree. Reach your own independent conclusions from the code — do not assume convergence.
