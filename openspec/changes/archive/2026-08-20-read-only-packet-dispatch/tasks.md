...
Largest slices are `internal/run/run_test.go` (Done-outcome matrix plus test-helper stubs) and `internal/worktree/worktree_test.go` (real-git helpers). Docs and CLI wiring are small.
...
- [x] 2.2 **GREEN** `internal/worktree/worktree.go`: add `HasUniqueCommits(ctx, worktreePath, primaryRoot)` using `git -C` on those two roots only. Honour a cancelled `ctx` the way `Create` does. `go test ./internal/worktree -count=1` green for 2.1.
...
- [x] 2.4 **GREEN** `internal/worktree/worktree.go`: add `PorcelainEmpty(ctx, worktreePath)` via `git -C worktreePath status --porcelain`. `go test ./internal/worktree -count=1` green.
...
- [x] 4.1 **RED** `cmd/lucind-ai/cli.go`, `cmd/lucind-ai/cli_test.go`: failing proof that production `run()` Deps sets `HasUniqueLaneCommits` and `PorcelainEmpty` to non-nil git-backed funcs. If the inlined `lucindrun.Deps{...}` literal (`cli.go:169`) has no observable seam, extract the smallest package-level constructor whose only consumer is `run()`'s `ExecuteBatch` call. Do not add an unused wrapper.
- [x] 4.2 **GREEN** `cmd/lucind-ai/cli.go`: `HasUniqueLaneCommits` closes over `primaryRoot` and calls `worktree.HasUniqueCommits`; `PorcelainEmpty` is `worktree.PorcelainEmpty`. `go test ./cmd/lucind-ai -count=1` green.
