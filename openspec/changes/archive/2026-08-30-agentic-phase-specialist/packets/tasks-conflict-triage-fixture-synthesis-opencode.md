---
id: tasks-conflict-triage-fixture-synthesis-opencode
executor: opencode
routed_by: synthesis of three parallel tasks lenses into one canonical tasks-opencode.md
model: openai/gpt-5.6-luna
allowed_paths: ["openspec/changes/conflict-triage-fixture/tasks-opencode.md", "openspec/changes/conflict-triage-fixture/tasks-opencode-synthesis-notes.md"]
---

# Packet tasks-conflict-triage-fixture-synthesis-opencode

**Tier:** A (human merge)
**Worktree:** ../lucind-ai-worktrees/tasks-conflict-triage-fixture-synthesis-opencode  ·  **Branch:** lucind/tasks-conflict-triage-fixture-synthesis-opencode

## Goal

Read the three tasks lens drafts for `conflict-triage-fixture`, verify their claims against the real code, arbitrate where they disagree, and produce one canonical `openspec/changes/conflict-triage-fixture/tasks-opencode.md` plus a separate synthesis notes file recording everything that did not make it in and why.
...
This lane's canonical tasks file is `openspec/changes/conflict-triage-fixture/tasks-opencode.md`,
...
`openspec/changes/conflict-triage-fixture/tasks.md`: it is outside this lane's `allowed_paths`,
does not exist in this worktree, and a lane that touches a path it was not granted is rejected at
integration regardless of how good its content is.
...
- `openspec/changes/conflict-triage-fixture/tasks-opencode.md` does not yet exist.
...
- A citation that does not resolve, points at unrelated code, or does not support the claim: **drop the claim from `tasks-opencode.md`** and record it under `## Dropped Citations` in the notes with what you found instead.
...
- A work unit from lens B or an acceptance row from lens C that maps to no task in lens A's checklist does not go into `tasks-opencode.md`. Record it under `## Decomposition Divergence` in the notes with what that lens assumed instead.
...
`tasks-opencode.md` MUST be under 1800 words. The three drafts total roughly 3000. Cutting is the job: a task line from lens A, its unit from lens B, and its proof from lens C are one entry, not three.
...
Any wave that cannot be green alone must be merged into its successor before `tasks-opencode.md` ships. Record what you merged and why under `## Coverage Gaps`.
...
`tasks-opencode.md` must satisfy this repository's tasks spine:
...
### `openspec/changes/conflict-triage-fixture/tasks-opencode.md`
...
`tasks-opencode.md` stays the human checklist. It is not the parse source for dispatch — an `apply-dag.yaml` sidecar is, when one is warranted. If lens B recommended no sidecar, say so in `tasks-opencode.md` and name the single-packet shape instead.

### `openspec/changes/conflict-triage-fixture/tasks-opencode-synthesis-notes.md`
...
`openspec/changes/conflict-triage-fixture/tasks-opencode.md` and `openspec/changes/conflict-triage-fixture/tasks-opencode-synthesis-notes.md` only.
...
**Commit `tasks-opencode.md` the moment it is written, before you begin the notes file.** Then write the
...
**Right after the first commit** (`tasks-opencode.md`, before you start the notes file):

```
./lucind-lane-check.sh --file openspec/changes/conflict-triage-fixture/tasks-opencode.md \
...
```
./lucind-lane-check.sh --file openspec/changes/conflict-triage-fixture/tasks-opencode-synthesis-notes.md \
...
narrating the same facts in prose. `tasks-opencode.md`'s own spine coverage (the eight items in step 6) is
substantive, not a fixed set of heading strings, so the script does not and cannot check it — that
judgment stays yours.

## Done criteria

- [ ] **`tasks-opencode.md` was committed as its own commit before `tasks-opencode-synthesis-notes.md` was started**, confirmed by the mid-flow `lucind-lane-check.sh` run reporting a clean `git status --porcelain`.
- [ ] **Every `file:line` citation surviving into `tasks-opencode.md` was opened and confirmed in this worktree**, and every dropped claim is listed under `## Dropped Citations`.
...
- [ ] **`tasks-opencode.md` exists, is under 1800 words, and substantively covers all eight spine items**, with anything missing reported under `## Coverage Gaps`.
- [ ] **`tasks-opencode-synthesis-notes.md` exists with exactly the four required sections**, each either populated or explicitly "None", confirmed by the final `lucind-lane-check.sh` run.
...
- The three `## Assumed decomposition` blocks are mutually irreconcilable and the spec and design do not choose between them. Write the notes file, leave `tasks-opencode.md` uncreated, and block.
