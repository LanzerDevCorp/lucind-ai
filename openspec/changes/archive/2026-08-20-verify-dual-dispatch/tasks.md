...
- **Cross-check with the independent `tasks-agy.md` draft**: agy's parallel estimate came in lower
  still, 500–750 lines, and independently reached the same conclusion — low risk, single PR, no
  chain needed. Both drafts agree this is the smallest of the three dual-executor-dispatch
  sibling changes by a wide margin (contrast `apply-dag-dispatch`'s corroborated 1700–3800-line,
  over-budget forecast). No delivery-strategy decision is required before a future apply phase
  for this change.
